...
基础分类是默认且始终启用的规则层，不存在需要用户主动打开的“高级接管模式”。新安装使用“内容优先、安全保守”预设：普通内容、列表详情、文档和媒体默认新标签页；首页与站点导航默认同标签页；分页、搜索、图片、SPA、表单和敏感行为默认保持原生。

### 五档基础预设

“精准、内容、广泛、深入、最广”现在只是批量填写分类值的预设，不再决定某类行为能否进入规则系统。应用预设前会显示变更预览；应用后，各分类仍可独立改为同标签页、新标签页或保持原生；语义子类和站点覆写还可继续继承上层结果。
